...
pusherServer
  ...
  .catch((err) =>
    console.error("[UAZAPI WEBHOOK] Pusher trigger failed:", err),
  );
```
...
const content =
  textContent || (mediaType ? (MEDIA_PLACEHOLDER[mediaType] ?? "[Mídia]") : "");
...
if (hasMedia) {
  (async () => {
    try {
      ...
      pusherServer
        .trigger(`workspace-${channel.workspaceId}`, "message-updated", {
          conversationId: conversation.id,
